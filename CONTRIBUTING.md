...
> [!NOTE]
> The provider requires Go 1.26 or later to build.
...
2. Create a `testacc.env` file in the project root with:
  ...
  Some tests require multi node cluster, a ZFS datastore or a spare disk to run. To include these (otherwise skipped) tests add additional configuration corresponding to the cluster.
