...
data "proxmox_file" "cloud_init_snippet" {
  node_name    = "pve"
  datastore_id = "local"
...
data "proxmox_file" "imported_file" {
  node_name    = "pve"
  datastore_id = "local"
  content_type = "import"
...
output "ubuntu_iso_id" {
  value = data.proxmox_file.ubuntu_iso.id
}

output "ubuntu_iso_size" {
  value = data.proxmox_file.ubuntu_iso.file_size
}

output "container_template_format" {
  value = data.proxmox_file.ubuntu_container_template.file_format
}
