...
func (c *Client) CreateIPSet(ctx context.Context, d *firewall.IPSetCreateRequestBody) error {
	err := c.DoRequest(
		ctx,
		http.MethodPost,
		fmt.Sprintf("nodes/%s/qemu/%d/firewall/ipset", url.PathEscape(c.NodeName), c.VMID),
		d,
		nil,
	)
	if err != nil {
...
func (c *Client) AddCIDRToIPSet(ctx context.Context, id string, d firewall.IPSetGetResponseData) error {
	err := c.DoRequest(
		ctx,
		http.MethodPost,
		fmt.Sprintf("nodes/%s/qemu/%d/firewall/ipset/%s",
			url.PathEscape(c.NodeName), c.VMID, url.PathEscape(id)),
		&d,
		nil,
	)
	if err != nil {
...
func (c *Client) DeleteIPSetContent(ctx context.Context, id string, cidr string) error {
	err := c.DoRequest(
		ctx,
		http.MethodDelete,
		fmt.Sprintf("nodes/%s/qemu/%d/firewall/ipset/%s/%s",
			url.PathEscape(c.NodeName), c.VMID, url.PathEscape(id), url.PathEscape(cidr)),
		nil,
		nil,
	)
	if err != nil {
