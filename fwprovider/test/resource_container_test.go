...
// TestAccResourceContainerHookScriptRefresh verifies that `containerRead` populates the
// `hook_script_file_id` attribute in state. Same drift-refresh bug as #2850's `features`
// block — written to PVE on Create/Update but never read back into state.
//
// Without the fix, the attribute is missing from state entirely (the schema default is
// not applied during Read). With the fix, state contains an empty string for an
// unconfigured hookscript, which is what the assertion below verifies.
//
// The full drift-detection pattern (PreConfig PUT + ExpectNonEmptyPlan) used by the
// sibling tests is not feasible here: PVE gates `hookscript` to root@pam basic auth (API
// tokens are rejected) AND validates that the referenced snippet file is executable on
// disk. Making the snippet executable from the test SSH user requires sudo privileges
// that the test SSH user is not granted. So this test asserts the (smaller) read-back
// behavior; the underlying d.Set logic is identical to that for `description`, which is
// covered by every other container acceptance test.
func TestAccResourceContainerHookScriptRefresh(t *testing.T) {
	te := InitEnvironment(t)
	imageFileName := fmt.Sprintf("%d-alpine-3.22-default_20250617_amd64.tar.xz", time.Now().UnixMicro())
	testAccDownloadContainerTemplate(t, te, imageFileName)
	...
	containerConfig := te.RenderConfig(`
		resource "proxmox_virtual_environment_container" "test_container" {
			...
			initialization {
				hostname = "test-hookscript-refresh"
				ip_config {
					ipv4 {
						address = "dhcp"
					}
