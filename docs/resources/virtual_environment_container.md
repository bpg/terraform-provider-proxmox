...
If the same apply also changes other configuration (e.g. a `mount_point` whose host path differs between
nodes), the provider shuts the container down, migrates it offline, applies the configuration change on the
target node, and then starts it. This avoids starting the container with configuration that no longer
matches its declared state, and avoids restarting it twice. No extra configuration is needed to get this
behavior.

Containers managed by Proxmox HA are migrated through the HA manager, and the provider waits for the
container to actually arrive and settle on the target node before continuing.
