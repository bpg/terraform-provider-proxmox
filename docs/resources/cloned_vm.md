...
This resource intentionally manages only a subset of VM configuration. The following are currently not managed and must be inherited from the source template (or managed via `proxmox_vm` with a `clone` block):
...
resource "proxmox_cloned_vm" "basic_clone" {
  node_name = "pve"
  name      = "basic-clone"
...
resource "proxmox_cloned_vm" "network_managed" {
  node_name = "pve"
  name      = "network-clone"
...
resource "proxmox_cloned_vm" "disk_managed" {
  node_name = "pve"
  name      = "disk-clone"
...
resource "proxmox_cloned_vm" "full_featured" {
  node_name   = "pve"
  name        = "production-vm"
...
resource "proxmox_cloned_vm" "test_clone" {
  node_name = "pve"
  name      = "test-vm"
...
resource "proxmox_cloned_vm" "pooled_clone" {
  node_name = "pve"
  name      = "pooled-vm"
...
resource "proxmox_cloned_vm" "imported" {
  # Import with: terraform import proxmox_cloned_vm.imported pve/123
