...
| `disk.size` (increase) | Applied online when possible; may still require manual reboot later for non-hotpluggable disk setups |
...
| `template` | `false -> true` converts in place; `true -> false` is unsupported |
...
For the `cloned_vm` resource, see the [Clone VM guide](clone-vm.md) and the [Choosing Between Clone Resources guide](migration-vm-clone.md) for details on the `delete` block and inherited device management.
