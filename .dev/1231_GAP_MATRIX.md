...
| vcpus (rehomed `cpu.hotplugged`)     | #14       | todo   | Per P3 + ADR-008 single-vs-map rule              |
...
| Capability                       | Target PR | Status | Notes                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------- | --------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| network_device (map-keyed block) | #10       | todo   | Per OQ2: read-only agent fields rehomed per-slot as `ipv4_addresses` (List), `ipv6_addresses` (List), `interface_name` (String) in BOTH resource and datasource. Per enabled rule: drop SDK `network_device[slot].enabled` (provider invention; per-slot `disconnected` → real PVE `link_down=1` covers soft-disable). Drop SDK top-level `mac_addresses` (per-slot `mac_address` covers it). |
...
| Capability                         | Target PR | Status | Notes                                                                                                                                                                                                                 |
| ---------------------------------- | --------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| agent                              | #13       | todo   | Per enabled rule: keep `agent.enabled` (real PVE sub-param, `url:"enabled,int"` in `CustomAgent`). Per OQ4: keep `agent.timeout` and `agent.wait_for_ip.{ipv4,ipv6}` as PVE pass-through.                             |
| numa (map-keyed block)             | #13       | todo   | Includes `numa.enabled` (rehomed `cpu.numa`). Per enabled rule: keep — real PVE param `numa=1` (top-level VM NUMA-emulation toggle, distinct from `numa[N]` topology slots).                                          |
| watchdog                           | #13       | todo   | Per enabled rule: drop SDK `watchdog.enabled` (pure schema-level invention; not in `CustomWatchdogDevice` struct). Block presence in HCL conveys "configured".                                                        |
| acpi                               | #14       | todo   | —                                                                                                                                                                                                                     |
| tablet_device                      | #14       | todo   | —                                                                                                                                                                                                                     |
| keyboard_layout                    | #14       | todo   | —                                                                                                                                                                                                                     |
| kvm_arguments                      | #14       | todo   | —                                                                                                                                                                                                                     |
...
| audio_device (single-nested block) | #17       | todo   | Single-nested per ADR-008 (joins `efi_disk`/`tpm_state`); forward-compat trade-off accepted. Per enabled rule: drop SDK `audio_device.enabled` (provider invention; `CustomAudioDevice.Enabled` has `url:"-"`).       |
| virtiofs (map-keyed block)         | #17       | todo   | Per enabled rule: drop SDK `virtiofs[slot].enabled` (provider invention; `CustomVirtualIODevice.Enabled` has `url:"-"`). Keep `backup` sub-field — real PVE param (`BackupEnabled` has `url:"backup,omitempty,int"`). |
...
> Design Q1–Q5 resolved during grilling. These are _new_ open questions
