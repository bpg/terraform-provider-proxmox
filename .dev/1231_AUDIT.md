...
| F29 | rename             | should-fix | —   | `cpu/resource_schema.go:83`      | ~~(Confirms P3) `cpu.hotplugged` exists. Drop in #3, rehome as top-level `vcpus` in #14.~~ **Revised 2026-04-23:** rename in place to `cpu.vcpus` (matches PVE API key + Processors UI). No rehome; stays inside the `cpu` sub-block. | #3 (rename)    |
| F30 | keep               | resolved   | —   | `cpu/resource_schema.go:101`     | ~~(Confirms P3) `cpu.numa` exists. Drop in #3, rehome as `numa.enabled` in #13.~~ **Revised 2026-04-23:** keep as `cpu.numa` (matches PVE Processors UI). No rehome.                                                                  | #3 (no change) |
...
| SDK key                       | SDK source                               | Status      | Target PR | Notes                                                                                                                                                                          |
| ----------------------------- | ---------------------------------------- | ----------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `cpu`                         | `vm.go:195`                              | done        | —         | In production. PR #3 ports to ADR-008. `hotplugged` renamed to `vcpus` in place (2026-04-23 revision); `numa` kept in the cpu block.                                           |
...
| ~~`vcpus` (top-level, rehomed `cpu.hotplugged`)~~ — superseded | (n/a)       | rescinded | —         | Revised 2026-04-23: `vcpus` stays inside the `cpu` sub-block (rename of `hotplugged` in PR #3). PVE Processors UI grouping decisive. |
...
| ------------------------------ | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | --------- |
| `cpu` (block)                  | Optional+Computed | absent unless the user set at least one inner field (empirical: PVE does not auto-populate anything on the wire — see 2026-04-24 revision) | n/a                                              | **Optional** (drop Computed at block level — when user provides nothing, PVE returns nothing)                  | #3        |
...
| `cpu.cores`                    | Optional+Computed | absent when unset (empirical 2026-04-24 — original "AUTO-POPULATED to 1" claim was wrong)                                                  | 1                                                | **Optional** — carve-out abandoned; see summary below                                                          | #3        |
| `cpu.flags`                    | Optional+Computed | absent                                                                                                                                     | none                                             | **Optional**                                                                                                   | #3        |
| `cpu.limit`                    | Optional+Computed | absent                                                                                                                                     | 0                                                | **Optional**                                                                                                   | #3        |
| `cpu.numa` (bool)              | Optional+Computed | absent                                                                                                                                     | 0                                                | **Optional** — stays in cpu block (revised 2026-04-23; matches PVE Processors UI grouping)                     | #3        |
| `cpu.sockets`                  | Optional+Computed | absent when unset (empirical 2026-04-24 — original "AUTO-POPULATED to 1" claim was wrong)                                                  | 1                                                | **Optional** — carve-out abandoned; see summary below                                                          | #3        |
...
| `cpu.vcpus` (was `hotplugged`) | Optional+Computed | absent                                                                                                                                     | 0                                                | **Optional** — renamed in place from `hotplugged` → `vcpus` (revised 2026-04-23; PVE API name + Processors UI) | #3        |

**cpu carve-out summary**: all 9 attributes go Optional-only. The originally-proposed `Optional+Computed` carve-out on `cores`/`sockets` was abandoned in PR #3 implementation once empirical testing showed PVE does **not** auto-populate them — and keeping Computed created an O+C null-in-config trap that blocked `attribute.CheckDeleteBody`. `hotplugged` was renamed to `vcpus` in place (2026-04-23); `numa` stays in the cpu block.
...
| Rename in place (`cpu.hotplugged` → `cpu.vcpus`; `cpu.numa` kept)                     | 2 (cpu.hotplugged → vcpus, cpu.numa)             | #3 (rename + keep)        |
...
| `vcpus` (was `hotplugged`) | `int64validator.Between(1, 1024)` (`cpu/resource_schema.go`)                                            | range           | **keep**  | Attribute renamed in place; validator carries over (revised 2026-04-23). Originally planned as drop-on-rehome.                                                                                                                                                                                                                                 | #3 (rename + keep) |
...
| keep (rename) | 1     | `cpu.hotplugged` → `cpu.vcpus` validator carried over (revised 2026-04-23)                                            |
...
| P3  | `cpu.numa` and `cpu.hotplugged` naming cleanup                                                                       | should-fix | #3 (rename `hotplugged` → `vcpus`; keep `numa` as-is) | ~~Original plan: rehome to top-level `vcpus` + `numa.enabled` block.~~ **Revised 2026-04-23:** cpu sub-block mirrors PVE "Processors" UI grouping, so these stay in `cpu`. The naming debt was only a misnomer on `hotplugged` — fixed by rename. |
